...
for i in range(n_signal):
	...
	V.append(ValueNetwork(num_features=6, hidden_size=256, learning_rate=0.001))
		...
		# Before the while loop, initialize the plot (outside the loop)
if render:
...
while i_episode<n_episode:
	...
	while steps<max_steps:
		...
		if steps%T==0:
			for i in range(n_agent):
				avg_utility = sum(u_bar) / len(u_bar)  # Average utility of all agents
				if avg_utility != 0:
					fairness_penalty = abs((u_bar[i] - avg_utility) / avg_utility)
				else:
					fairness_penalty = 0
				meta_rewards[i].append(1 - fairness_penalty)  # Reward proximity to average		
		...
		# Inside the while loop, replace the plotting code with:
		if render:
			ax.cla()  # Clear the axis to update positions without closing the window
			...
			# Plot resources as green points
			for i in range(n_resource):
				ax.scatter(resource[i][0], resource[i][1], color='green')
			...
			plt.pause(0.1)  # Update the plot with a slight pause
...
plt.show()  # Display the final frame (optional)
